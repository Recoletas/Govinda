...
if [[ "$START_VLLM" == "1" ]]; then
    if [[ "$ALLOW_EXISTING_VLLM" != "1" ]]; then
        if curl -fsS --noproxy 127.0.0.1 "http://127.0.0.1:${PORT}/health" >/dev/null 2>&1; then
            echo "ERROR: existing vLLM responds on port $PORT; stop it first or set ALLOW_EXISTING_VLLM=1" >&2
            exit 1
        fi
        if pgrep -f "vllm.entrypoints|vllm serve|VLLM::EngineCore" >/dev/null; then
            echo "ERROR: existing vLLM process found; stop it first or set ALLOW_EXISTING_VLLM=1" >&2
